...
public:
    ...
    int solve(int i, string s, vector<int>& dp) {
        ...
        if (i >= s.size())
            return 1; // When we have gone out of bounds of the string, it means
                      // that we have decoded the entire string, and therefore
                      // we return 1
        else if (s[i] == '0')
            return 0; // We neither can decode 0 individually, neither with
                      // another character like 06, therefore return 0 whenever
                      // we encounter 0

        else if (i == s.size() - 1)
            return 1; // when we reach the last character that means there can
